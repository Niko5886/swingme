// @component EventsToast - Тост: Събития (няма информация)
'use client';
import React from 'react';
import { Star } from 'lucide-react';

interface EventsToastProps { show: boolean; onClose: () => void; }

export default function EventsToast({ show, onClose }: EventsToastProps) {
  if (!show) return null;
  return (
    <div aria-live="polite" className="fixed bottom-4 right-4 z-[60] animate-[slide-in_0.2s_ease-out]">
      <div className="flex items-start gap-3 rounded-xl border border-purple-500/30 bg-gradient-to-br from-slate-900/95 to-purple-900/95 px-4 py-3 shadow-2xl backdrop-blur">
        ...
        <div className="text-sm text-purple-100">Няма предстоящи събития...</div>
